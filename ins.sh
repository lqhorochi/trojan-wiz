...
function dl_install_trojan(){
	...
	#if [ "$tj" = "" ];then
		trojan="https://github.com/trojan-gfw/trojan/releases/download/v1.15.0/trojan-1.15.0-linux-amd64.tar.xz"
...
while [ 1 ]
	do
		opt=$(whiptail --title "Trojan GFW 配置向导 v2.1" --menu "请选择证书模式" 15 60 3 \
		"1" "使用IP自签发证书" \
